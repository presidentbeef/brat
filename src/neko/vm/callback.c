...
EXTERN value val_callEx( value vthis, value f, value *args, int nargs, value *exc ) {
	...
	if( val_is_int(f) )
		val_throw(alloc_string("Invalid call - function is an integer?"));
	if( val_tag(f) == VAL_PRIMITIVE ) {
		...
		if( nargs == ((vfunction*)f)->nargs ) {
			...
			val_throw(alloc_string("Invalid call - Arguments don't match up in primitive"));		
		if( ret == NULL )
		...
		if( nargs == ((vfunction*)f)->nargs )  {
			...
			val_throw(alloc_string("Invalid call - it's a function but args don't match."));
	} else
		val_throw(alloc_string("Invalid call - who knows what this thing is?"));
	if( exc ) {
		neko_process_trap(vm);
		memcpy(&vm->start,&oldjmp,sizeof(jmp_buf));	
	}
