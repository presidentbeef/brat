...
class BratStdlibTests < Test::Unit::TestCase
	...
	def test_set_new
		assert_result "true", 'squish import "set"; s = set.new; s.empty?'
		assert_result "3", 'squish import "set"; s = set.new 1,1,2,3; s.length'
		assert_result "3", 'squish import "set"; s = set.new [1,1,3,4]; s.length'
	end

	def test_set_add
		assert_result "true", 'squish import "set"; s = set.new; s.add 1; s.include? 1'
		assert_result "1", 'squish import "set"; s = set.new 1; s.add 1; s.length'
		assert_result "2", 'squish import "set"; s = set.new 1; s.add "1"; s.length'
		assert_result "2", 'squish import "set"; s = set.new "a"; s.add "b"; s.length'
	end

	def test_set_delete
		assert_result "false", 'squish import "set"; s = set.new; a = new; s.add a; s.delete a; s.include? a'
		assert_result "1", 'squish import "set"; s = set.new 1; a = []; s.add a; s.delete a; s.length'
		assert_result "1", 'squish import "set"; s = set.new 1; s.add "1"; s.delete 1; s.length'
		assert_result "1", 'squish import "set"; s = set.new "a"; s.add "b"; s.delete "b"; s.length'
		assert_result "true", 'squish import "set"; s = set.new; a = new; s.add a; s.delete a; s.empty?'
	end
